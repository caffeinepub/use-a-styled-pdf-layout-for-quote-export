...
export function useUpdateRateCard() {
  ...
  return useMutation({
    mutationFn: async (items: RateCardItem[]) => {
      if (!actor) throw new Error('Actor not available');
      await actor.updateRateCard(items);
    },
    onSuccess: async () => {
      // Invalidate and refetch to ensure UI updates
      await queryClient.invalidateQueries({ queryKey: ['rateCard'] });
      await queryClient.refetchQueries({ queryKey: ['rateCard'] });
    },
  });
}
...
export function useUpdateAccountManagers() {
  ...
  return useMutation({
    mutationFn: async (managers: AccountManager[]) => {
      if (!actor) throw new Error('Actor not available');
      await actor.updateAccountManagers(managers);
    },
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['accountManagers'] });
      await queryClient.refetchQueries({ queryKey: ['accountManagers'] });
    },
  });
}
